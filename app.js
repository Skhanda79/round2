...
app.get("/view-all-events-admin", async (req, res) => {
    try {
        // Find the user with the given email
        const adminUser = await userCollection.findOne({ lastName: "Admin1" });
        if (!adminUser) {
            return res.status(404).send("User not found.");
        }

        // Fetch all events for the found user
        const adminUserEvents = await eventCollection.find({ userID: adminUser._id.valueOf() });
        res.render('view_all_events_admin', { events: adminUserEvents });
    } catch (err) {
        res.status(500).json({ message: 'Server Error' });
    }
});
